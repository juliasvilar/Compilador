    ...
    for funcao in funcoes:
        if funcao.nome == nome_funcao:
            if len(funcao.tipos_parametros) != len(tipos_argumentos):
                return False
            for tipo_parametro, tipo_argumento in zip(funcao.tipos_parametros,
                                                      tipos_argumentos):
    ...
    for i, linha in enumerate(linhas):
        if 'int ' in linha or 'float ' in linha:
            ...
            if not verificarDeclaracaoVariavel(variavel):
                print(
                    f"Erro semântico: variável '{variavel}' não declarada na linha {i+1}"
                )
                return False
            ...
            if not verificarCompatibilidadeTipos(tipo_variavel, tipo_valor):
                print(
                    f"Erro semântico: atribuição de tipo incompatível para a variável '{variavel}' na linha {i+1}"
                )
                return False
        elif 'soma(' in linha:
            ...
            for parametro in parametros:
                if not verificarDeclaracaoVariavel(parametro.strip()):
                    print(
                        f"Erro semântico: variável '{parametro.strip()}' não declarada na linha {i+1}"
                    )
                    return False
                tipo_parametro = obterTipoVariavel(parametro.strip())
                if not verificarCompatibilidadeTipos(tipo_parametro, 'int'):
                    print(
                        f"Erro semântico: tipo incompatível para a função 'soma' na linha {i+1}"
                    )
                    return False
    return True
